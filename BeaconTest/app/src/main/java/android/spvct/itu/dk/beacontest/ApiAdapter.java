            ...
            if (conn.getResponseCode() == HttpsURLConnection.HTTP_OK) {
                ...
                if( root.isJsonArray() ) {
                    ...
                    for (int i = 0; i < rootArr.size(); i++) {
                        ...
                        String uid = element.get("uid").toString();

                            BeaconEntity be = new BeaconEntity();
                            be.setUUID(element.get("uid").toString());
                            //responseData[i] = be;
                            responseData[i] = gson.fromJson(element, returnType);


                        //Log.i("TTTTTTTT", "Response data UID  = " + uid);
                    }
                } else if ( root.isJsonObject() ) {
                    responseData = getReturnInstanceArray(1);
                    responseData[0] = gson.fromJson(root, returnType);
                    //Log.i("TTTTTTTT", "Response data object  = " + responseData[0]);
                }
                in.close();
            } else {
